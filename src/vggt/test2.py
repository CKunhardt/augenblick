...
class MeshGenerationPipeline:
    ...
    def _generate_mesh_tsdf(self, predictions: Dict) -> o3d.geometry.TriangleMesh:
        ...
        for i in range(len(depth_maps)):
            ...
            color_o3d = o3d.geometry.Image((images[i] * 255).astype(np.uint8))
...
class VGGTPhotogrammetryPipeline:
    def __init__(self, max_images: int = 128, max_resolution: int = 512, batch_size: Optional[int] = None):
        """
        ...
        Args:
            ...
            batch_size: If set, process images in smaller batches
        """
    ...
    def run_inference_batched(self, images: torch.Tensor) -> Dict:
        """
        Run inference with optional batching for memory efficiency
        """
        if self.batch_size is None or images.shape[0] <= self.batch_size:
            return self._run_single_batch(images)
        
        # Process in batches
        print(f"Processing {images.shape[0]} images in batches of {self.batch_size}")
        
        all_predictions = {}
        for i in range(0, images.shape[0], self.batch_size):
            end_idx = min(i + self.batch_size, images.shape[0])
            batch = images[i:end_idx]
            
            print(f"Processing batch {i//self.batch_size + 1}: images {i}-{end_idx-1}")
            batch_predictions = self._run_single_batch(batch)
            ...
            if i == 0:
                ...
                for key, value in batch_predictions.items():
                    if isinstance(value, np.ndarray):
                        all_predictions[key] = np.concatenate([all_predictions[key], value], axis=0)
                    else:
                        # For non-concatenatable items, keep the first batch's values
                        pass
...
def main():
    # Configure for your GPU memory (adjust these based on your results)
    pipeline = VGGTPhotogrammetryPipeline(
        max_images=128,          # Reduce if still getting OOM
        max_resolution=448,     # VGGT typically uses 512, but 448 saves memory
        batch_size=8           # Process 8 images at a time, reduce if needed
    )
    ...
    try:
        ...
        mesh_methods = ['tsdf', 'poisson', 'alpha_shapes']
        ...
        print(f"OOM error: {e}")
        print("Try reducing max_images, max_resolution, or batch_size further")
